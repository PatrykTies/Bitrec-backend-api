(function($){
   ...
   var textTween4 = d3.select('#tween4').append('svg').attr("class", "countdown")
    			    .attr("height", numbersHeight)
	      		   .attr("width", numbersWidth)
	      		   .attr("transform", 'translate(0,100)')
	        	   .append("text").attr('value','0').style('fill','white');
   var textTween5 = d3.select('#tween5').append('svg').attr("class", "countdown")
   	...
   	$('#fullpage').fullpage({
   		...
   		anchors: ['top','services','stats','smartHRO','manage','contact'],
   		menu: '#side_menu',
   		afterLoad: function(link, index){
   			if(link == 'contact'){
				$('#okrag').delay(2000).fadeOut();
   			}
   			if(link == 'top'){
   			...
   			if(link == 'stats'){
   				render(innerRadius,outerRadius);
   				renderEthnic(innerRadius,outerRadius);
   				renderPositions(innerRadius,outerRadius);
   				tweenWorkers(textTween3,500,0);
   				tweenWorkers(textTween4,700,1);
   				tweenWorkers(textTween5,900,2);
	...
	function render(innerRadius, outerRadius) {
